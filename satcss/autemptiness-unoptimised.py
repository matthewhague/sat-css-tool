...
_hashable_zero = HashableZ3(0)

class AutEmptinessUnoptimisedException(Exception):
    pass
...
def isempty(aut, data=None):
    ...
    # some code for trying to speed up rule construction, work in progress
    if t > 0.5:
        print "Slow check of ", data, " took ", t, " seconds."
        #set_option(max_args=10000000, max_lines=1000000, max_depth=10000000, max_visited=1000000)
        #print solver
        #if res == sat:
        #    print solver.model()
...
def _sel_has_pseudo_elements(sel, cons, neg_cons = None):
...
def _reset_pos_var():
    """To avoid position variable numbers being too high, call this when
    starting again with an empty set of clauses so pos var overlap won't matter
    """
    global _next_pos_var
    _next_pos_var = 0
...
def _new_pos_var():
    ...
    """
    global _next_pos_var
    v = Int("p" + str(_next_pos_var))
    _next_pos_var += 1
    return v

def _pos_cons_satisfiable(pos_cons, pv, pd, pdstar, fixed_pos = None):
    """
    :param pos_cons:
        A set of HashableZ3 equations (x + d + 1) &c.
    :param pv:
        The pv of the next node that has one (Z3 Int var)
    :param pd:
    ...
    pos = HashableZ3(_free_pos_var) if fixed_pos is None else fixed_pos

    for c in pos_cons:
        _emp_z3.add(c.z3)

    _emp_z3.add(_get_pos_constraint(pos, pv, pd, pdstar).z3)
...
def _get_pos_constraint(pv, next_pv, pd, pdstar):
    """
    :param pv:
        The position (or position var) of the current node (HashableZ3)
    :param next_pv:
        The position of the next node with a position var
        or None if there is none (HashableZ3 variable)
    :param pd:
        Minimum distance between pv and next_pv
    :param pdstar:
        True iff the distance can be arbitrarily pumped
    :returns:
        A HashableZ3 constraint enforcing pv is right distance from next_pv
    """
    if next_pv is None:
        # always satisfiable!
        return HashableZ3(And())
    if pdstar:
        return HashableZ3(Exists([_nvar],
                                   And(next_pv.z3 == pv.z3 + _nvar + pd + _nvar,
                                       _nvar >= 0)))
    else:
        return HashableZ3(next_pv.z3 == pv.z3 + pd)


def _get_sel_info(sel):
    """
    :param sel:
    ...
    :returns:
        ...
        pv is the position variable in the constraints, HashableZ3 (or None if not used)
    """
        ...
        if stype == "Negation":
            ...
            if subtype == "Element":
                ...
                if s.subselector.ident == "first-child":
                    if pv is None:
                        pv = HashableZ3(_new_pos_var())
                        pos_cons.add(HashableZ3(pv.z3 >= 0))
                        pos_cons.add(HashableZ3(_nlast - pv.z3 >= 0))
                    pos_cons.add(HashableZ3(pv.z3 > 0))
                if s.subselector.ident == "last-child":
                    if pv is None:
                        pv = HashableZ3(_new_pos_var())
                        pos_cons.add(HashableZ3(pv.z3 >= 0))
                        pos_cons.add(HashableZ3(_nlast - pv.z3 >= 0))
                    pos_cons.add(HashableZ3(_nlast - pv.z3 > 0))
                if s.subselector.ident == "only-child":
                    if pv is None:
                        pv = HashableZ3(_new_pos_var())
                        pos_cons.add(HashableZ3(pv.z3 >= 0))
                        pos_cons.add(HashableZ3(_nlast - pv.z3 >= 0))
                    pos_cons.add(HashableZ3(Or(pv.z3 > 0,
                                               _nlast - pv.z3 == 0)))
                else:
                    neg_ps.add(s.subselector.ident)
            elif subtype == "Function" and s.subselector.name == "nth-child":
                a, b = cssfile.get_fun_sel_coefs(s.subselector)
                if pv is None:
                    pv = HashableZ3(_new_pos_var())
                    pos_cons.add(HashableZ3(pv.z3 >= 0))
                    pos_cons.add(HashableZ3(_nlast - pv.z3 >= 0))
                pos_cons.add(HashableZ3(_not_nth(pv.z3, a, b)))
            elif subtype == "Function" and s.subselector.name == "nth-last-child":
                a, b = cssfile.get_fun_sel_coefs(s.subselector)
                if pv is None:
                    pv = HashableZ3(_new_pos_var())
                    pos_cons.add(HashableZ3(pv.z3 >= 0))
                    pos_cons.add(HashableZ3(_nlast - pv.z3 >= 0))
                pos_cons.add(HashableZ3(_not_nth(_nlast - pv.z3, a, b)))
        elif stype == "Pseudo":
            if s.ident == "first-child":
                if pv is None:
                    ...
                    pos_cons.add(HashableZ3(_nlast - pv.z3 >= 0))
                pos_cons.add(HashableZ3(pv.z3 == 0))
            if s.ident == "last-child":
                if pv is None:
                    pv = HashableZ3(_new_pos_var())
                    pos_cons.add(HashableZ3(pv.z3 >= 0))
                    pos_cons.add(HashableZ3(_nlast - pv.z3 >= 0))
                pos_cons.add(HashableZ3(_nlast - pv.z3 == 0))
            if s.ident == "only-child":
                if pv is None:
                    pv = HashableZ3(_new_pos_var())
                    pos_cons.add(HashableZ3(pv.z3 >= 0))
                    pos_cons.add(HashableZ3(_nlast - pv.z3 >= 0))
            ...
            ids.add(s.id)
        elif stype == "Function" and s.name == "nth-child":
            a, b = cssfile.get_fun_sel_coefs(s)
            if pv is None:
                pv = HashableZ3(_new_pos_var())
                pos_cons.add(HashableZ3(pv.z3 >= 0))
                pos_cons.add(HashableZ3(_nlast - pv.z3 >= 0))
            pos_cons.add(HashableZ3(_nth(pv.z3, a, b)))
        elif stype == "Function" and s.name == "nth-last-child":
            a, b = cssfile.get_fun_sel_coefs(s)
            if pv is None:
                pv = HashableZ3(_new_pos_var())
                pos_cons.add(HashableZ3(pv.z3 >= 0))
                pos_cons.add(HashableZ3(_nlast - pv.z3 >= 0))
            pos_cons.add(HashableZ3(_nth(_nlast - pv.z3, a, b)))
        s = s.selector
...
def _nth(x, a, b):
    ...
    """
    if a != 0:
        return Exists([_nvar], x == a * _nvar + b)
    else:
        return x == b
...
class _Z3NormAutEnc:
    ...
    def __has_pseudo_elements(self, cons):
        ...
        """
        for t in self.aut:
            if _sel_has_pseudo_elements(t.node_selector, cons):
                return True
        return False
    ...
    def __get_tran_dummies(self, t):
        ...
        # requirement
        is_of_type = _sel_has_pseudo_elements(sel, _of_type_cons)
        ...
        if stype == "Element":
            ...
            if stype == "Negation":
                ...
                if sel.ident == "first-child":
                    ...
                    cons.append(_nth(self.nchild[i], 0, 1))
                elif sel.ident == "last-child":
                    cons.append(Not(self.psvars["root"][i]))
                    cons.append(_nth(self.nlchild[i], 0, 1))
                ...
                if sel.name == "nth-child":
                    ...
                    cons.append(Not(self.psvars["root"][i]))
                    cons.append(_nth(self.nchild[i], a, b))
                elif sel.name == "nth-last-child":
                    a, b = cssfile.get_fun_sel_coefs(sel)
                    cons.append(Not(self.psvars["root"][i]))
                    cons.append(_nth(self.nlchild[i], a, b))
        ...
        if stype == "Element":
            ...
            if sel.namespace is not None:
                return Or(self.psvars["root"][i],
                          self.nsvars[i] != self.nsvals[sel.namespace])
            if sel.element is not None:
                return Or(self.psvars["root"][i],
                          self.evars[i] != self.evals[sel.element])
        elif stype == "Pseudo":
            if sel.ident == "first-child":
                return Or(self.psvars["root"][i],
                          _not_nth(self.nchild[i], 0, 1))
            elif sel.ident == "last-child":
                return Or(self.psvars["root"][i],
                          _not_nth(self.nlchild[i], 0, 1))
            elif sel.ident == "only-child":
                return Or(
                    self.psvars["root"][i],
                    _not_nth(self.nchild[i], 0, 1),
            ...
            if sel.name == "nth-child":
                a, b = cssfile.get_fun_sel_coefs(sel)
                return Or(self.psvars["root"][i],
                          _not_nth(self.nchild[i], a, b))
            elif sel.name == "nth-last-child":
                a, b = cssfile.get_fun_sel_coefs(sel)
                return Or(self.psvars["root"][i],
                          _not_nth(self.nlchild[i], a, b))
            elif sel.name == "nth-of-type":
                a, b = cssfile.get_fun_sel_coefs(sel)
                return Or(self.psvars["root"][i],
    ...
    def __nth_of_type(self, a, b, i):
        ...
        """
        return self.__of_type(a, b, i, self.ntchild, _nth)
    ...
    def __not_nth_of_type(self, a, b, i):
        ...
        :returns:
            Z3 encoding expressing the selector
        """
        return self.__of_type(a, b, i, self.ntchild, _not_nth)
    ...
    def __nth_last_of_type(self, a, b, i):
        ...
        :returns:
            Z3 encoding expressing the selector
        """
        return self.__of_type(a, b, i, self.nltchild, _nth)
    ...
    def __not_nth_last_of_type(self, a, b, i):
        ...
        """
        return self.__of_type(a, b, i, self.nltchild, _not_nth)
