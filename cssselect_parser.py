...
"""
    ...
    Modified for satcss by Matthew Hague

        * Fixed attribute representation:
            a.namepace = None for atts [*|a op v]
            a.namespace = "" for atts [|a op v] or [a op v]
            a.namespace = ns for atts [ns|a op v]
"""
...
class Selector(object):
    ...
    def __eq__(self, other):
        if isinstance(other, self.__class__):
            return (self.pseudo_element == other.pseudo_element and
                    self.parsed_tree == other.parsed_tree)
        else:
            return False

    def __hash__(self):
        return hash(self.pseudo_element) + hash(self.parsed_tree)
...
class Class(object):
    ...
    def __hash__(self):
        return hash(self.selector) + hash(self.class_name)
...
class Function(object):
    ...
    def __hash__(self):
        return hash(self.selector) + hash(self.name) + len(self.arguments)
...
class Pseudo(object):
    ...
    def __hash__(self):
        return hash(self.selector) + hash(self.ident)
...
class Negation(object):
    ...
    def __eq__(self, other):
        if isinstance(other, self.__class__):
            return (self.selector == other.selector and
                    self.subselector == other.subselector)
        else:
            return False

    def __hash__(self):
        return hash(self.selector) + hash(self.subselector)
...
class Element(object):
    ...
    def __hash__(self):
        return (hash(self.namespace) + hash(self.element))
...
class Hash(object):
    ...
    def __hash__(self):
        return (hash(self.selector) + hash(self.id))
...
class CombinedSelector(object):
    ...
    def __hash__(self):
        return (hash(self.selector) +
                hash(self.combinator) +
                hash(self.subselector))
